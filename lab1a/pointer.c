...
int pointer_size() {
  ...
  return (long)&ptr_ptr1 - (long)&ptr_ptr2;
}
...
int change_value() {
  ...
  //
  //           21: 000010101
  // Shift 4 left: 101010000
  //  XOR with 15: 101011111

  *(int_array + 5) = (21 << 4) ^ 15;

  return *(int_array + 5);  // change this line so you don't need []
}
...
int within_same_block(int* ptr1, int* ptr2) {
  // to solve:
  // drop the 6 LSBs and compare
  // address will be in the same 64-bit block if equal
...
int within_array(int* int_array, int size, int* ptr) {
  // if pointer address is less than first array address
  // pointer address is not one of the array addresses
  if (!(((ptr - int_array) >> 31) == 0)) {
    return ((((ptr - int_array) >> 31) == 0));
  }

  // return true if pointer address is less than or equal to first
  // array address + size - 1, AKA the last address in the array
  return ((size - 1 - (ptr - int_array)) >> 31) == 0; 
}
...
int endian_experiment(int* ptr) {
  char* byte_ptr;
  // point char* to int* to move bit by bit
  byte_ptr = (char*) ptr;

  // lowest address has LSB
  *byte_ptr = 0b01110111;
  *(byte_ptr + 1) = 0b01011100;
  *(byte_ptr + 2) = 0b00000101;
