...
unsigned short get_section(unsigned long* aisle, int index) {
  // shift bits-of-interest to the 16 LSB position and apply mask
  return (*aisle >> (SECTION_WIDTH * index)) & SECTION_MASK;
}
...
unsigned short get_id(unsigned long* aisle, int index) {
  // get section, apply mask and shift 10 bits right
  return (get_section(aisle, index) & ID_MASK) >> SPACE_WIDTH;
}
...
void set_section(unsigned long* aisle, int index, unsigned short new_section) {
  ...
  // eg. index = 1 -> 00000000FFFF0000
  mask = mask << (SECTION_WIDTH * index);
  
  // FFFFFFFF0000FFFF
  mask = ~mask;

  // aisle = XXXXXXXX0000XXXX
  *aisle = *aisle & mask;

  // aisle = (XXXXXXXX0000XXXX) XOR (00000000NNNN0000)
  //
  // aisle = XXXXXXXXNNNNXXXX
  *aisle = *aisle ^ (((unsigned long) new_section) << (SECTION_WIDTH * index));
}
...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  // if new spaces has true bits beyond the mask, it's too big
  if (new_spaces > SPACES_MASK) {
    return;
  }

  unsigned short new_section;
  new_section = (get_section(aisle, index) & ~SPACES_MASK) ^ new_spaces;
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  if ((new_id & ~(ID_MASK >> SPACE_WIDTH)) > 0) {
    return;
  }

  unsigned short new_section;
  new_section = (get_section(aisle, index) & ~ID_MASK) ^ (new_id << SPACE_WIDTH);
  set_section(aisle, index, new_section);
}
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  unsigned short new_spaces;
  new_spaces = get_spaces(aisle, index) ^ (0x0001 << space_index);
...
unsigned short num_items(unsigned long* aisle, int index) {
  ...
  // check each bit in spaces
  for (char i = 0; i < SPACE_WIDTH; i += 1) {
    // increment if LSB is 1
    count += spaces & 1;
    // shift bits to right
    spaces = spaces >> 1;
  }
...
void add_items(unsigned long* aisle, int index, int n) {
  ...
  // for each bit in spaces
  for (char i = 0; i < SPACE_WIDTH; i += 1) {
    // if there are still items to add AND the current index "i" is free
    if ((n > 0) & ~((spaces >> i) & 1)) {
...
void remove_items(unsigned long* aisle, int index, int n) {
  // flip spaces bits (eg. 0b1101 -> 0b0010)
  set_spaces(aisle, index, ~get_spaces(aisle, index) & SPACES_MASK);
  
  // add items (eg. 0b0010 -> 0b0111)
  add_items(aisle, index, n);

  // flip spaces bits again (eg. 0b0111 -> 0b1000)
  set_spaces(aisle, index, ~get_spaces(aisle, index) & SPACES_MASK);
}
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  ...
  unsigned short left_shifted = get_spaces(aisle, index) << n;
  unsigned short right_shifted = get_spaces(aisle, index) >> (SPACE_WIDTH - n);

  set_spaces(aisle, index, (left_shifted | right_shifted) & SPACES_MASK);
}
...
void rotate_items_right(unsigned long* aisle, int index, int n) {
  ...
  rotate_items_left(aisle, index, SPACE_WIDTH - n);
}
